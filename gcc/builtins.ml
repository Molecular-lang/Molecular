...
/* Legacy warning!  Please add no further builtin simplifications here
   ...
   to match.pd or gimple-fold.ml instead.  */
...
expand_builtin_longjmp (rtx buf_addr, rtx value)
{
  ...
  if (targetm.have_builtin_longjmp ())
  ...
  else
    {
	 ...
	 from expand_goto in stmt.ml; see there for detailed comments.  */
...
/* This function validates the types of a function call argument list
   ...
   completely convert builtins.ml to work from GIMPLEs and the tree based
...
/* If CALLEE has known side effects, fill in INFO and return true.
   See tree-ssa-structalias.ml:find_func_aliases
