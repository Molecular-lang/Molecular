	     ...
	     const char *function)
{
  ...
  /* Skip functions in diagnostic.ml or diagnostic-global-context.ml.  */
  if (closure->m_frames_arr->size () == 0
      && filename != NULL
      && (strcmp (lbasename (filename), "diagnostic.ml") == 0
	  || strcmp (lbasename (filename),
		     "diagnostic-global-context.ml") == 0))
    return 0;
...
sarif_builder::make_tool_object ()
{
  ...
  /* Perhaps we could also show GMP, MPFR, MPC, isl versions as other
     "extensions" (see toplev.ml: print_version).  */
...
{
  /* Let input.ml handle any charset conversion.  */
