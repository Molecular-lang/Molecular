...
/* Loop and basic block vectorizer.
  ...
  - tree-vect-loop.ml - loop specific parts such as reductions, etc. These are
    used by drivers (1) and (2).
  - tree-vect-loop-manip.ml - vectorizer's loop control-flow utilities, used by
    drivers (1) and (2).
  - tree-vect-slp.ml - BB vectorization specific analysis and transformation,
    used by drivers (2) and (3).
  - tree-vect-stmts.ml - statements analysis and transformation (used by all).
  - tree-vect-data-refs.ml - vectorizer specific data-refs analysis and
    manipulations (used by all).
  - tree-vect-patterns.ml - vectorizable code patterns detector (used by all)
  ...
  Here's a poor attempt at illustrating that:
     ...
     loop_vect()  loop_aware_slp()  slp_vect()
          |        /           \          /
          |       /             \        /
          tree-vect-loop.ml  tree-vect-slp.ml
                | \      \  /      /   |
                |  \      \/      /    |
                |   \     /\     /     |
                |    \   /  \   /      |
         tree-vect-stmts.ml  tree-vect-data-refs.ml
                       \      /
                    tree-vect-patterns.ml
*/
