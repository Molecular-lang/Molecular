...
/* Return the value of the integer term in X, if one is apparent;
   ...
   This is used in cse.ml with the `related_value' field.  */
...
side_effects_p (const_rtx x)
{
  ...
  switch (code)
    ...
    case CLOBBER:
      /* Reject CLOBBER with a non-VOID mode.  These are made by combine.ml
...
/* Return the number of registers that a subreg REG with REGNO
   expression refers to.  This is a copy of the rtlanal.ml:subreg_nregs
	  ...
	  int opno, bool speed)
{
  ...
  switch (code)
    ...
    case US_DIV:
      ...
      break;
    case USE:
      /* Used in combine.ml as a marker.  */
      total = 0;
      break;
    default:
