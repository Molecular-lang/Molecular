...
/* Implementation of selective scheduling approach.
   ...
   the same routine from ia64.ml.

   Dependence analysis changes
   ===========================

   We augmented the sched-deps.ml with hooks that get called when a particular
   ...
   There are parts of haifa-sched.ml, sched-deps.ml, and sched-rgn.ml that are
...
/* Returns register class of the output register in INSN.
   ...
   Code adopted from regrename.ml::build_def_use.  */
...
/* Choose the register among free, that is suitable for storing
   ...
   Some code is adapted from regrename.ml (regrename_optimize).
      ...
      if (HARD_REGISTER_NUM_P (orig_regno))
	{
	  ...
	  if (!TEST_HARD_REG_BIT (reg_rename_p->unavailable_hard_regs,
				  orig_regno))
	    {
		 ...
		 that in sched-deps.ml sched_analyze_reg().  */
			    ...
			    struct reg_rename *reg_rename_p)
{
  ...
  if (target_available)
    ...
    /* Check only if we haven't scheduled something on the previous fence,
       ...
       was created in sched-deps.ml for insns with destination registers that
...
can_speculate_dep_p (ds_t ds)
{
  ...
  {
    /* FIXME: make sched-deps.ml produce only those non-hard dependencies,
...
sel_rank_for_schedule (const void *x, const void *y)
{
  ...
  if (spec_info != NULL && spec_info->mask != 0)
    /* This code was taken from haifa-sched.ml: rank_for_schedule ().  */
    {
      ds_t ds1, ds2;
      dw_t dw1, dw2;
      int dw;
