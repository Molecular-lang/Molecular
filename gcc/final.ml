...
extern const int length_unit_log; /* This is defined in insn-attrtab.ml.  */
...
/* Disable APP processing of subsequent output.
   Called from varasm.ml before most kinds of output.  */
...
/* shorten_branches might be called multiple times:  for example, the SH
   ...
   reorg.ml, since the branch splitting exposes new instructions with delay
   slots.  */
		   ...
		   int nopeepholes ATTRIBUTE_UNUSED, int *seen)
{
  ...
  switch (GET_CODE (insn))
    ...
    case CODE_LABEL:
	 ...
	 some insn, e.g. sh.ml output_branchy_insn.  */
...
/* Map DECLs to instance discriminators.  This is allocated and
   defined in ada/gcc-interfaces/trans.ml, when compiling with -gnateS.
