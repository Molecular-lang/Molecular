  ...
  for (line_map_uint_t i = 0; i < LINEMAPS_MACRO_USED (line_table); i++)
      ...
      for (unsigned int i = 0; i < MACRO_MAP_NUM_MACRO_TOKENS (map); i++)
	{
	  ...
	  /* I'm typically seeing 4 uninitialized entries at the end of
	     0xafafafaf.
	     This appears to be due to macro.ml:replace_args
...
/* Various selftests involve constructing a line table and one or more
   ...
   and others use zero
   - the fallback modes within line-map.ml: there are various threshold
   values for location_t beyond line-map.ml changes
...
public:
  ...
  /* The ordering of these fields matters.
     ...
     since the filenames in input.ml's input cache are owned by the
...
#endif
    /* Detect and record errors emitted by libcpp/charset.ml:init_iconv_desc
...
test_lexer_string_locations_concatenation_1 (const line_table_case &case_)
{
  ...
  /* Simulate c-lex.ml's lex_string in order to record concatenation.  */
...
/* Ensure that we are fail gracefully if something attempts to pass
   ...
   when c-format.ml erroneously used the indicated one-character
