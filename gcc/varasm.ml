...
/* Return the section for function DECL.
   ...
   NULL_TREE under some circumstances by dbxout.ml at least.
...
assemble_external (tree decl ATTRIBUTE_UNUSED)
{
  ...
  /* This function should only be called if we are expanding, or have
     expanded, to RTL.
     Ideally, only final.ml would be calling this function, but it is
...
switch_to_comdat_section (section *sect, tree decl)
{
  ...
  /* Neither OBJECT_FORMAT_PE, nor OBJECT_FORMAT_COFF is set here.
     ...
     gcc/config/i386/winnt.ml: mingw_pe_unique_section.  */
  if (TARGET_PECOFF)
    {
      char *name;
