...
cp_common_type (tree t1, tree t2)
{
  ...
  if (code1 != REAL_TYPE)
	  ...
	  || same_type_p (TYPE_MAIN_VARIANT (t2), long_integer_type_node))
	{
	 ...
	 types (enforced in toplev.ml).  Prefer the unsigned type. */
...
/* Wrapper around cp_common_type that is used by c-common.ml and other
                              ...
                              tsubst_flags_t complain)
{
  ...
  /* For MIN_EXPR or MAX_EXPR, fold-const.ml has arranged things so that
...
cp_build_addr_expr_1 (tree arg, bool strict_lvalue, tsubst_flags_t complain)
{
  ...
  if (TREE_CODE (arg) == COMPONENT_REF && type_unknown_p (arg)
	...
	return error_mark_node;
	 ...
	 extension.
	 See comment in class.ml:resolve_address_of_overloaded_function for
