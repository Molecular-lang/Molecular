...
/* An exception is an event that can be "thrown" from within a
   ...
   During initial gimplification (gimplify.ml) these are lowered to the
   ...
   During pass_lower_eh (tree-eh.ml) we record the nested structure
   ...
   During pass_lower_eh_dispatch (tree-eh.ml), which is run after
   ...
   During pass_lower_resx (tree-eh.ml), which is run near the end
   ...
   During pass_expand (cfgexpand.ml), we generate REG_EH_REGION notes
   ...
   During pass_convert_to_eh_region_ranges (except.ml), we transform
   ...
   output_function_exception_table (except.ml) to emit the tables with
   ...
   output_call_frame_info (dwarf2out.ml) emits the required unwind data.  */
...
/* Routines to generate the exception tree somewhat directly.
   These are used from tree-eh.ml when processing exception related
