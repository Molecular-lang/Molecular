  ...
  if (bb_seq (bb) != NULL)
    {
	 ...
	 eliminate_unnecessary_stmts() in tree-ssa-dce.ml for more
	 details.  */
...
verify_gimple_debug (gimple *stmt ATTRIBUTE_UNUSED)
{
  /* There isn't much that could be wrong in a gimple debug stmt.  A
     ...
     insns, and are converted to debug notes by var-tracking.ml.  */
  return false;
}
...
/* These tests directly create CFGs.
   Compare with the static fns within tree-cfg.ml:
     - build_gimple_cfg
