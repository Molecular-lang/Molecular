...
/* Change of width--truncation and extension of integers or reals--
   ...
   narrowing is always done with a NOP_EXPR:
     In convert.ml, convert_to_integer[_maybe_fold].
     In c-typeck.ml, build_binary_op_nodefault (boolean ops),
	and c_common_truthvalue_conversion.
     In expr.ml: expand_expr, for operands of a MULT_EXPR.
     In fold-const.ml: fold.
     In tree.ml: get_narrower and get_unwidened.
	...
	if (!is_volatile || !is_complete || TREE_ADDRESSABLE (type))
          {
            /* Emit a warning (if enabled) when the "effect-less" INDIRECT_REF
               ...
               - an expression with TREE_NO_WARNING set. (For an example of
                 such expressions, see build_over_call in call.ml.)
               - automatic dereferencing of references, since the user cannot
                 control it. (See also warn_if_unused_value() in c-common.ml.)  */
...
convert_force (tree type, tree expr, int convtype, tsubst_flags_t complain)
{
  ...
  /* From typeck.ml convert_for_assignment */
...
/* The routines below this point are carefully written to conform to
   ...
   closely.  Although they are used only in pt.ml at the moment, they
