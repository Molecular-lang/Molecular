...
/* Functions and data structures for #pragma interface.
   ...
   its main body.  I.e., if this is file "foo.ml", and class `bar'
   is defined in "foo.ml", then we say that "foo.ml implements bar".

   All main input files "implement" themselves automagically.

   `#pragma interface' means that unless this file (of the form "foo.h"
   is not presently being included by file "foo.ml", the
   CLASSTYPE_INTERFACE_ONLY bit gets set.  The effect is that none
   of the vtables nor any of the inline functions defined in foo.h
   will ever be output.

   There are cases when we want to link files such as "defs.h" and
   "main.ml".  In this case, we give "defs.h" a `#pragma interface',
   and "main.ml" has `#pragma implementation "defs.h"'.  */
...
interface_strcmp (const char* s)
{
  ...
  for (ifiles = impl_file_chain; ifiles; ifiles = ifiles->next)
	...
	return 0;
      ...
      /* Don't get faked out by xxx.yyy.ml vs xxx.zzz.ml.  */
