...
trees_out::tree_node (tree t)
{
  ...
  if (!is_initial_scan () && writing_local_entities)
	...
	local_decl = t;
	 ...
	 reduces the module size slightly and means that much less of pt.ml
  ...
  if (!decl_p)
    {
	 ...
	 pt.ml optimizes instantiation lookup using a complicated
...
/* True iff TMPL has an explicit instantiation definition.
   ...
   This is local to module.ml because register_specialization skips adding most
...
module_state::read_cluster (unsigned snum)
{
  ...
  /* Look, function.ml's interface to cfun does too much for us, we
