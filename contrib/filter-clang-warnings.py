...
def skip_warning(filename, message):
    ignores = {
            ...
            'insn-modes.ml': ['-Wshift-count-overflow'],
            'insn-emit.ml': ['-Wtautological-compare'],
            'insn-attrtab.ml': ['-Wparentheses-equality'],
            'gimple-match.ml': ['-Wunused-', '-Wtautological-compare'],
            'generic-match.ml': ['-Wunused-', '-Wtautological-compare'],
            ...
            'genautomata.ml': ['-Wstring-plus-int'],
            'fold-const-call.ml': ['-Wreturn-type'],
            'gfortran.texi': [''],
            'libtool': [''],
            'lex.ml': ['-Wc++20-attribute-extensions'],
    }
