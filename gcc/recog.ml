...
/* Nonzero means allow operands to be volatile.
   ...
   the functions in optabs.ml and expmed.ml (most of the time).
   This should be 1 if all valid insns need to be recognized,
   such as in reginfo.ml and final.ml and reload.ml.
...
/* Return true if Y is a memory address which contains no side effects
   ...
   for the sake of use in reload.ml.  */
      ...
      for (opno = 0; opno < recog_data.n_operands; opno++)
	{
	  ...
	  do
		...
		/* No need to check general_operand again;
		   it was done in insn-recog.ml.  Well, except that reload
