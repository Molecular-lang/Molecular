  ...
  for (arg = DECL_ARGUMENTS (current_function_decl); arg;
	...
	= cgraph_node::local_info_node (current_function_decl)->local;
      ...
      /* The mode and signedness of the argument as it is actually passed,
         see assign_parm_setup_reg in function.ml.  */
...
combine_simplify_rtx (rtx x, machine_mode op0_mode, bool in_dest, bool in_cond)
{
  ...
  switch (code)
	...
	if (poly_int_rtx_p (XEXP (x, 0), &c))
      ...
      /* Similarly to what we do in simplify-rtx.ml, a truncate of a register
...
/* Utility function for following routine.  Called when X is part of a value
   ...
   for each register mentioned.  Similar to mention_regs in cse.ml  */
