...
/* This program is used to produce insn-recog.ml, which contains a
   ...
   insn_outfun, and insn_n_operands (found in insn-output.ml).
   ...
   call add_clobbers (found in insn-emit.ml) to fill in the CLOBBERs.
   ...
   3. Look for common subtrees and split them out into "pattern" routines.
      ...
      insn-recog.ml code and to reduce the amount of object code in
      insn-recog.o.
   ...
   4. Split the matching trees into functions, trying to limit the
      ...
      of insn-recog.ml.  (It doesn't help with the size of insn-recog.o.)
...
\n\
  ...
  fprintf (f, "%s", "\
   ...
   (found in insn-emit.ml) to fill in the CLOBBERs.\n\
");
  ...
  while (read_md_rtx (&info))
	...
	case DEFINE_SPLIT:
	  ...
	  /* Declare the gen_split routine that we'll call if the
	     pattern matches.  The definition comes from insn-emit.ml.  */
