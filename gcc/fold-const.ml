	...
	case ROUND_DIV_EXPR:
	  if (flag_complex_method == 0)
	  {
	    /* Keep this algorithm in sync with
	       tree-complex.ml:expand_complex_div_straight().
	  ...
	  {
	    /* Keep this algorithm in sync with
               tree-complex.ml:expand_complex_div_wide().
		 ...
		 tree op0, tree op1)
{
  ...
  switch (code)
	  ...
	  && negate_expr_p (op0))
	{
	 ...
	 after the last round to changes to the DIV code in expmed.ml.  */
  ...
  *slot = expr;
  ...
  if (TREE_CODE_CLASS (code) == tcc_declaration
  ...
  else if (warning_suppressed_p (expr) && (DECL_P (expr) || EXPR_P (expr)))
    {
	 ...
	 that and change builtins.ml etc. instead - see PR89543.  */
      size_t sz = tree_size (expr);
      buf = XALLOCAVAR (union tree_node, sz);
      memcpy ((char *) buf, expr, sz);
      buf->base.nowarning_flag = 0;
