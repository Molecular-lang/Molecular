...
/* Various things, organized as linked lists, needed both in
   gengtype.ml & in gengtype-state.ml files.  */
...
/* Functions reading and writing the entire gengtype state, called from
   main, and implemented in file gengtype-state.ml.  */
...
/* Token codes.  */
/* Keep 'gengtype-parse.ml:token_names', 'gengtype-parse.ml:token_value_format'
   in sync.  */
enum gty_token
{
  EOF_TOKEN = 0,
