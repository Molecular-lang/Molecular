...
/* Shorter alias name for the above function for use in gimplify.ml
   only.  */
			   ...
			   bool want_value, bool notify_temp_creation)
{
  ...
  switch (TREE_CODE (type))
	...
	/* TODO.  There's code in cp/typeck.ml to do this.  */
...
/* This function handles several cases where clauses on a mapping directive
   ...
   c-omp.ml:c_omp_adjust_map_clauses and marked the appropriate decl
  ...
  if (!struct_map_to_clause || struct_map_to_clause->get (base) == NULL)
      ...
      else if (attach_detach)
	{
	  ...
	  /* For OpenMP semantics, we don't want to implicitly allocate
	     ...
	     omp-low.ml is able to rewrite the struct properly.
	...
	*inner = insert_node_pos;
	...
	unordered = true;
	 ...
	 is not a decl.  The FIRSTPRIVATE_* nodes are removed in omp-low.ml
	...
	case OMP_CLAUSE_MAP:
	  ...
	  if (TREE_CODE (decl) == TARGET_EXPR)
		...
		move_attach = true;
		 ...
		 argument since omp-low.ml wants to see the decl itself.  */
	      if (OMP_CLAUSE_MAP_KIND (c) == GOMP_MAP_STRUCT)
		break;
	...
	if (OMP_CLAUSE_CODE (c) == OMP_CLAUSE_MAP)
	  switch (OMP_CLAUSE_MAP_KIND (c))
	    ...
	    case GOMP_MAP_TO_PSET:
		 ...
		 clause (see trans-openmp.ml:gfc_trans_omp_clauses).  */
	      break;
	    case GOMP_MAP_POINTER:
...
/* Convert the GENERIC expression tree *EXPR_P to GIMPLE.  If the
   ...
   GIMPLE_TEST_F points to a function that takes a tree T and
       ...
       caller.  The GIMPLE predicates are in gimple.ml.
		      ...
		      gimple_seq *post_p ATTRIBUTE_UNUSED)
{
  ...
  if (have_va_type == NULL_TREE
    ...
    /* Handle 'Case 1: Not an array type' from c-common.ml/build_va_arg.  */
    have_va_type
