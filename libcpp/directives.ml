...
/* Check if we have a known directive.  INDENTED is true if the
   ...
   to save unnecessarily exporting dtable etc. to lex.ml.  Returns
...
_cpp_handle_directive (cpp_reader *pfile, bool indented)
{
  ...
  if (dir)
	...
	pfile->mi_valid = false;
	 ...
	 -fpreprocessed mode only if the # is in column 1.  macro.ml
...
/* Process a #define directive.  Most work is done in macro.ml.  */
static void
