				 ...
				 bool *may_tailcall, bool call_from_thunk_p)
{
  ...
  for (argpos = 0; argpos < num_actuals; i--, argpos++)
	...
	type = TREE_TYPE (first_field (type));
	 ...
	 with those made by function.ml.  */
	...
	*must_preallocate = true;
	...
	ADD_PARM_SIZE (*args_size, args[i].locate.size.var);
	 ...
	 corresponding handling of incoming arguments in function.ml
...
expand_call (tree exp, rtx target, int ignore)
{
  ...
  for (pass = try_tail_call ? 0 : 1; pass < 2; pass++)
      ...
      if (pass && (flags & ECF_MALLOC))
	{
	 ...
	 function.ml:setjmp_warnings that it should complain if
			   ...
			   machine_mode outmode, int nargs, rtx_mode_t *args)
{
  ...
  for (count = 0; count < nargs; count++, argnum--)
      ...
      if (! (reg != 0 && partial == 0))
	{
	  ...
	  /* Indicate argument access so that alias.ml knows that these
	     values are live.  */
	  if (argblock)
	    ...
	    {
	      /* When arguments are pushed, trying to tell alias.ml where
  ...
  /* For calls to `setjmp', etc., inform function.ml:setjmp_warnings
