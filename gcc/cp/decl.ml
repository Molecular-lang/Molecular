...
/* Push the declarations of builtin types into the global namespace.
   ...
   is code in c-common.ml that uses identifier_global_value to look up
...
cxx_init_decl_processing (void)
{
  ...
  /* Ensure attribs.ml is initialized.  */
  init_attributes ();
		...
		tree asmspec_tree, int flags, cp_decomp *decomp)
{
  ...
  if (init && VAR_P (decl))
      ...
      if (init_const_expr_p)
	{
      ...
      /* This is handled mostly by gimplify.ml, but we have to deal with
...
check_class_member_definition_namespace (tree decl)
{
  ...
  /* We check for problems with specializations in pt.ml in
  ...
  if (attr)
    {
	 ...
	 used in c-common.ml:handle_cleanup_attribute when performing
