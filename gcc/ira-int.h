...
struct ira_emit_data
{
  /* TRUE if the allocno assigned to memory was a destination of
     removed move (see ira-emit.ml) at loop exit because the value of
  ...
  /* Non NULL if we remove restoring value from given allocno to
     MEM_OPTIMIZED_DEST at loop exit (see ira-emit.ml) because the
...
/* Correspondingly overall cost of the allocation, cost of the
   ...
   for pseudo-register live range splitting (see ira-emit.ml).  */
