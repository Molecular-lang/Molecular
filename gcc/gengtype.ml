...
/* Table of headers to be included in gtype-desc.ml that are generated
   during the build.  These are identified as "./<filename>.h".  */
const char **build_headers;
size_t num_build_headers;

/* A number of places use the name of this "gengtype.ml" file for a
...

/* File mapping routines.  For each input file, there is one output .ml file
...
open_base_files (void)
{
  ...
  /* gtype-desc.ml is a little special, so we create it here.  */
  {
    ...
    gtype_desc_c = create_file ("GCC", "gtype-desc.ml");
...
/* File rule action handling *.ml files.  */
...
struct file_rule_st files_rules[] = {
  ...
  { DIR_PREFIX_REGEX "c-family/([[:alnum:]_-]*)\\.ml$",
    REG_EXTENDED, NULL_REGEX,
    "gt-c-family-$3.h", "c-family/$3.ml", NULL_FRULACT},

  { DIR_PREFIX_REGEX "c-family/([[:alnum:]_-]*)\\.h$",
    REG_EXTENDED, NULL_REGEX,
    "gt-c-family-$3.h", "c-family/$3.h", NULL_FRULACT},

  /* Both c-lang.h & c-tree.h gives gt-c-c-decl.h for c-decl.ml !  */
  { DIR_PREFIX_REGEX "c/c-lang\\.h$",
    REG_EXTENDED, NULL_REGEX, "gt-c-c-decl.h", "c/c-decl.ml", NULL_FRULACT},

  { DIR_PREFIX_REGEX "c/c-tree\\.h$",
    REG_EXTENDED, NULL_REGEX, "gt-c-c-decl.h", "c/c-decl.ml", NULL_FRULACT},

  /* cp/cp-tree.h gives gt-cp-tree.h for cp/tree.ml !  */
  { DIR_PREFIX_REGEX "cp/cp-tree\\.h$",
    REG_EXTENDED, NULL_REGEX,
    "gt-cp-tree.h", "cp/tree.ml", NULL_FRULACT },

  /* cp/decl.h & cp/decl.ml gives gt-cp-decl.h for cp/decl.ml !  */
  { DIR_PREFIX_REGEX "cp/decl\\.[ch]$",
    REG_EXTENDED, NULL_REGEX,
    "gt-cp-decl.h", "cp/decl.ml", NULL_FRULACT },

  /* cp/name-lookup.h gives gt-cp-name-lookup.h for cp/name-lookup.ml !  */
  { DIR_PREFIX_REGEX "cp/name-lookup\\.h$",
    REG_EXTENDED, NULL_REGEX,
    "gt-cp-name-lookup.h", "cp/name-lookup.ml", NULL_FRULACT },

  /* cp/parser.h gives gt-cp-parser.h for cp/parser.ml !  */
  { DIR_PREFIX_REGEX "cp/parser\\.h$",
    REG_EXTENDED, NULL_REGEX,
    "gt-cp-parser.h", "cp/parser.ml", NULL_FRULACT },

  /* objc/objc-act.h gives gt-objc-objc-act.h for objc/objc-act.ml !  */
  { DIR_PREFIX_REGEX "objc/objc-act\\.h$",
    REG_EXTENDED, NULL_REGEX,
    "gt-objc-objc-act.h", "objc/objc-act.ml", NULL_FRULACT },

  /* objc/objc-map.h gives gt-objc-objc-map.h for objc/objc-map.ml !  */
  { DIR_PREFIX_REGEX "objc/objc-map\\.h$",
    REG_EXTENDED, NULL_REGEX,
    "gt-objc-objc-map.h", "objc/objc-map.ml", NULL_FRULACT },

  /* General cases.  For header *.h and *.ml files, we
   * need special actions to handle the language.  */

  /* Source *.ml files are using get_file_gtfilename to compute their
     output_name and get_file_basename to compute their for_name
     through the source_dot_cc_frul action.  */
  { DIR_PREFIX_REGEX "([[:alnum:]_-]*)\\.ml$",
    REG_EXTENDED, NULL_REGEX, "gt-$3.h", "$3.ml", source_dot_cc_frul},

  /* Common header files get "gtype-desc.ml" as their output_name,
...
/* Special file rules action for handling *.h header files.  It gives
   "gtype-desc.ml" for common headers and corresponding output
		   ...
		   char**pforname ATTRIBUTE_UNUSED)
{
  ...
  if (lang_index >= 0)
  ...
  else
    {
	 ...
	 output_name is "gtype-desc.ml" file.  The calling function
	 get_output_file_with_visibility will find its outf_p.  */
      free (*poutname);
      *poutname = xstrdup ("gtype-desc.ml");
      DBGPRINTF ("special 'gtype-desc.ml' for inpname %s",
...
/* Special file rules action for handling *.ml source files using
...
close_output_files (void)
{
  ...
  for (of = output_files; of; of = of->next)
      ...
      if (!is_file_equal (of))
	{
	  ...
	  /* Back up the old version of the output file gt-FOO.ml as
	     BACKUPDIR/gt-FOO.ml~ if we have a backup directory.  */
	  if (backup_dir)
	    {
