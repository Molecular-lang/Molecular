...
ix86_function_arg_regno_p (int regno)
{
  ...
  /* TODO: The function should depend on current function ABI but
     builtins.ml would need updating then. Therefore we use the
     default ABI.  */
  ...
  if (TARGET_SEH)
    {
	 ...
	 area, see the SEH code in config/i386/winnt.ml for the rationale.  */
...
/* Try machine-dependent ways of modifying an illegitimate address
   ...
   This macro is used in only one place: `memory_address' in explow.ml.
   ...
   See comments by legitimize_pic_address in i386.ml for details.  */
...
/* This is called from dwarf2out.ml via TARGET_ASM_OUTPUT_DWARF_DTPREL.
