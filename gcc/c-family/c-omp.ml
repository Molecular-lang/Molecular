					    ...
					    c_omp_region_type ort)
{
  ...
  switch (addr_tokens[i + 1]->u.access_kind)
	...
	if (decl_p && target_p && !declare_target_p)
	  {
	    /* It appears that omp-low.ml mishandles cases where we have a
	...
	/* The code handling "firstprivatize_array_bases" in gimplify.ml is
	...
	/* For OpenACC, use FIRSTPRIVATE_POINTER for decls even on non-compute
	   ...
	   gimplify.ml, but doing it this way maintains diagnostic
	   behaviour.  */
