...
/* An attribute for annotating formatting printing functions that use
   ...
   format codes (see pretty-print.ml), with additional codes for middle-end
   specific entities (see dumpfile.ml).  */
...
/* Different tree dump places.  When you add new tree dump places,
   extend the DUMP_FILES array in dumpfile.ml.  */
...
/* Bit masks to control dumping. Not all values are applicable to all
   ...
   the DUMP_OPTIONS array in dumpfile.ml. The TDF_* flags coexist with
...
/* The following API calls (which *don't* take a "FILE *")
   ...
   dump_* (MSG_*) --> dumpfile.ml --> items --> (a.1) dump_file
                                       |   `-> (a.2) alt_dump_file
                                       |
                                       `--> (b) optinfo
