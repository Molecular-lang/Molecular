...
/* Instruction scheduling pass.  This file, along with sched-deps.ml,
   contains the generic parts.  The actual entry point for
   the normal instruction scheduling pass is found in sched-rgn.ml.
...
/* Implement a circular buffer to delay instructions until sufficient
   ...
   than maximal time of instruction execution computed by genattr.ml on
...
/* Use this sel-sched.ml friendly function in reorder2 instead of increasing
...
increase_insn_priority (rtx_insn *insn, int amount)
{
  if (!sel_sched_p ())
    {
      /* We're dealing with haifa-sched.ml INSN_PRIORITY.  */
      if (INSN_PRIORITY_KNOWN (insn))
	  INSN_PRIORITY (insn) += amount;
    }
  else
    {
      /* In sel-sched.ml INSN_PRIORITY is not kept up to date.
...
try_ready (rtx_insn *next)
{
  ...
  /* * if !(old_ts & SPECULATIVE) (e.g. HARD_DEP or 0), then insn might
     ...
     * But if (old_ts & SPECULATIVE), then we are pretty sure that insn
     ...
     control-speculative NEXT could have been discarded by sched-rgn.ml
			     ...
			     basic_block second_bb)
{
  ...
  /* TODO: The actual probability can be determined and is computed as
     ...
     in sel-sched.ml `check_ds' in create_speculation_check.  */
