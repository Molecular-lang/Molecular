...
/* This file contains subroutines used only from the file reload1.ml.
	     ...
	     int opnum, enum reload_type type)
{
  ...
  if (in != 0 && GET_CODE (in) == SUBREG
	...
	/* This is supposed to happen only for paradoxical subregs made by
	   combine.ml.  (SUBREG (MEM)) isn't supposed to occur other ways.  */
	  ...
	  for (i = 0; i < noperands; i++)
	      ...
	      while (GET_CODE (operand) == SUBREG)
		{
		  ...
		  if (CONSTANT_P (operand)
			 ...
			 and gets extended when loaded from memory, combine.ml has
...
#if 0 /* This might screw code in reload1.ml to delete prior output-reload
...
/* Return nonzero if register in range [REGNO, ENDREGNO)
   ...
   This is similar to refers_to_regno_p in rtlanal.ml except that we
...
/* Nonzero if modifying X will affect IN.  If X is a register or a SUBREG,
   ...
   This function is similar to reg_overlap_mentioned_p in rtlanal.ml except
...
/* Check the insns before INSN to see if there is a suitable register
   ...
   This function is used by jump.ml as well as in the reload pass.
