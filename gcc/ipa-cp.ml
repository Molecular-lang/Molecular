...
/* Interprocedural constant propagation (IPA-CP).
   ...
   This stage is currently performed by call graph code (mainly in cgraphunit.ml
   and tree-inline.ml) according to instructions inserted to the call graph by
   the second stage.  */
...
/* Reset all state within ipa-cp.ml so that we can rerun the compiler
