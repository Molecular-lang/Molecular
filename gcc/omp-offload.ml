					 ...
					 vec<constructor_elt, va_gc> *v_ctor)
{
  ...
  for (unsigned i = 0; i < len; i++)
	...
	= is_var
      ...
      /* See also omp_finish_file and output_offload_tables in lto-cgraph.ml.  */
  ...
  if (targetm_common.have_named_sections)
      ...
      for (unsigned i = 0; i < num_funcs; i++)
	{
	  ...
	  /* See also add_decls_addresses_to_decl_constructor
	     and output_offload_tables in lto-cgraph.ml.  */
	  if (!in_lto_p && !symtab_node::get (it))
	    continue;
  ...
  if (is_oacc_kernels && is_oacc_kernels_parallelized)
    {
	 ...
	 also tree-parloops.ml:create_parallel_loop.  */
